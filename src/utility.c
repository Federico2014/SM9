...
/*
  ...
  msgLength: Length of message in bytes
...
void KDF(BYTE *k, BYTE *msg, unsigned int msgLength, unsigned int klen)
{
	...
	ct = 1;              // Initialize counter to 1
	v = 256;             // SM3 output is 256 bits
...
void Hash_1(CBigInt *h1, BYTE *msg, unsigned int len, CBigInt n)
{
	...
	if(Cmp(n,BN.n)== 0)  // Hash_1 parameter n is usually N (order of the group)
	{
		hlen = 320;      // hlen = 8*ceil(5*(log_2 n)/32)
		...
		HA.m_nLength = 10;
		for(i = 0; i<10; i++)          // Convert Ha array to big integer
		...
		printf("Hash1 error: unhandled exception\n");
}
...
void Hash_2(CBigInt *h1, BYTE *msg, unsigned int len, CBigInt n)
{
	...
	if(Cmp(n,BN.n)== 0)  // Hash_2 parameter n is usually N (order of the group)
	{
		hlen = 320;      // hlen = 8*ceil(5*(log_2 n)/32)
		hlen_v = 2;      // hlen_v = floor(hlen/v);
		...
		printf("Hash2 error: unhandled exception\n");
}
