...
void KA_Demo()
{
	...
	// System master key and user key generation
	start = clock();
	Get(&ke,"02E65B0762D042F51F0D23542B13ED8CFA2E9A0E7206361E013A283905E31F",HEX);  // Master private key
	P_multiply(&P_pub_e,P1,ke);                                // Master public key
	...
	Add_Big_Big(&t1,t1,ke);            // t1 is calculated modulo N, so no reduction needed
	while(Cmp(t1,BN.ZERO)==0)
	{
		// Need to regenerate master private key and public key
	}
	Inv(&t1_inv,t1,BN.n);
	...
	Add_Big_Big(&t3,t3,ke);          // t3 is calculated modulo N, so no reduction needed
	while(Cmp(t1,BN.ZERO)==0)
	{
		// Need to regenerate master private key and public key
	}
	...
	// Key agreement protocol phase
	// Steps A1-A4
	start = clock();
	Hash_1(&t3, msg2, len2, BN.n);
	P_multiply(&QB,P1,t3);
	P_add(&QB,QB,P_pub_e);
	Get(&rA,"5879DD1D51E175946F23B1B41E93BA31C584AE59A426EC1046A4D03B06C8",HEX);  // Generate random rA
	P_multiply(&RA,QB,rA);
	P_normorlize(&RA,RA);
	...
	P_multiply(&QA,P1,t1);
	P_add(&QA,QA,P_pub_e);
	Get(&rB,"018B98C44BEF9F8537FB7D071B2C928B3BC65BD3D69E1EEE213564905634FE",HEX);  // Generate random rB
	P_multiply(&RB,QA,rB);
	P_normorlize(&RB,RB);
	if(!P_isOnBNCurve(RA))
	{
	    printf("*********Key agreement protocol B4 step verification failed***********\n");
		exit(0);
	}
	...
	F12_exp(&g2,g2,rB);
	F12_exp(&g3,g1,rB);
	length = len1 + len2 - 2; // Byte length of IDA and IDB
	length += 64*2;        // Byte length of RA and RB
	length += 384*3;    // Byte length of g1, g2, g3, each 12th extension element needs 12*32 = 384 bytes
	...
	// Optional: Calculate Hash(g2||g3||IDA||IDB||RA||RB), note Hash uses SM3 algorithm
	len4 = length - 384;
	...
	// Optional step: Calculate SB = Hash(0x82||g1||Hash(g2||g3||IDA||IDB||RA||RB))
	SB_len = 1 + 384 + 32;
	...
	if(!P_isOnBNCurve(RB))
	{
	    printf("*********Key agreement protocol A5 step verification failed***********\n");
		exit(0);
	}
	...
	F12_exp(&g3_,g2_,rA);
	// Optional: Calculate Hash(g2_||g3_||IDA||IDB||RA||RB), note Hash uses SM3 algorithm
	cur = 0;
   ...
   // Optional step: Calculate S1 = Hash(0x82||g1_||Hash(g2_||g3_||IDA||IDB||RA||RB))
	msg_SB[0] = 0x82;
	...
	printf("\n********************************************\n");
	printf("*********SM9 Key Agreement Protocol Demo***********\n");
	printf("********************************************\n");
	printf("***********************************\n");
	...
	printf("Entity A identifier: %s\n",IDA);
	printf("t1 = H1(IDA||hid,N) + ke:\n %s\n",Put(t1,HEX));
	printf("t2 = ke * t1^(-1):\n %s\n",Put(t2,HEX));
	printf("deA = [t2]P2\n");
	P2_toString(deA,HEX);

	printf("Entity B identifier: %s\n",IDB);
	...
	printf("***********************************\n");
	printf("**********2. Key Agreement Protocol**********\n");
	printf("***********************************\n");
	printf("*****Steps A1-A4*****\n");
	...
	if(Bytes_Equal(SB,S1,32))
		printf("\nS1=SB, A6 step verification passed\n");
	else
	{
		printf("\nS1!=SB, A6 step verification failed\n");
		exit(0);
	}
	...
	if(Bytes_Equal(S2,SA,32))
		printf("\nS2=SA, B8 step verification passed\n");
	else
	{
		printf("\nS2!=SA, B8 step verification failed\n");
		exit(0);
	}

	printf("\nSM9 key agreement protocol system master key and user key generation time: %f ms\n",time1);
	printf("SM9 key agreement protocol time: %f ms\n",time2);
